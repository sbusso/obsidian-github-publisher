 ...
 */
export async function shareFolderRepo(
 ...
 */
export function addSubMenuCommandsFolder(
	plugin: GithubPublisher,
	item: MenuItem,
	folder: TFolder,
	branchName: string,
	...
	//@ts-ignore
	const subMenu = Platform.isDesktop
		? (item.setSubmenu() as Menu)
		: originalMenu;
...
export function addMenuFile(
	...
	menu.addItem(item => {
		...
		if (
			allKeysFromFile.length > 1 ||
			(repoFrontmatter instanceof Array && repoFrontmatter.length > 1)
		) {
 ...
 * @return {Menu} - The submenu created
 */
export function subMenuCommandsFile(
	plugin: GithubPublisher,
	item: MenuItem,
	file: TFile,
	branchName: string,
	...
	if (activatedRepoCommands.length > 0) {
		activatedRepoCommands.forEach(otherRepo => {
			if (
				otherRepo.shareKey === repo?.shareKey ||
				frontmatter?.[otherRepo.shareKey]
			) {
				subMenu.addItem(item => {
					item
						.setTitle(
