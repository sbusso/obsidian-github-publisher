...
export class ChooseRepoToRun extends FuzzySuggestModal<Repository> {
	...
	getItems(): Repository[] {
		...
		if (this.type === 'file') {
			if (
				this.plugin.settings.plugin.shareAll?.enable &&
		...
		if (repoFound.length === 0) return this.plugin.settings.github.otherRepo;
		return repoFound;
	}
	getItemText(item: Repository): string {
		return item.smartKey;
...
export class SuggestOtherRepoCommandsModal extends FuzzySuggestModal<GithubPublisherCommands> {
	...
	getItems(): GithubPublisherCommands[] {
		...
		if (
			this.plugin.settings.upload.autoclean.enable &&
			this.plugin.settings.upload.behavior !== FolderSettings.fixed
		) {
			cmd.push({
