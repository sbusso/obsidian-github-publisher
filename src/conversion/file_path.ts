...
function createObsidianPath(
	...
	const defaultFolder =
		repoFrontmatter?.path?.defaultName &&
		repoFrontmatter.path.defaultName.length > 0
			? repoFrontmatter.path.defaultName
...
function folderNoteIndexYAML(
	...
	const category =
		repoFrontmatter?.path?.category?.value ??
		getCategory(frontmatter, settings, repoFrontmatter?.path);
	...
	if (!settings.upload.folderNote.enable)
		return regexOnFileName(fileName, settings);
	if (
		fileName.replace('.md', '').toLowerCase() === parentCatFolder?.toLowerCase()
	)
...
function createFrontmatterPath(
	...
	const folderNote = folderNoteIndexYAML(
		fileName,
		frontmatter,
		settings,
		repoFrontmatter
...
export function regexOnFileName(
	...
	const uploadSettings = settings.upload;
	if (
		(fileName === uploadSettings.folderNote.rename &&
...
export function getReceiptFolder(
	...
	if (file.extension === 'md') {
		...
		if (!repoFrontmatter)
			repoFrontmatter = getRepoFrontmatter(settings, otherRepo, frontmatter);
		repoFrontmatter =
			repoFrontmatter instanceof Array ? repoFrontmatter : [repoFrontmatter];
		let targetRepo = repoFrontmatter.find(
		...
		if (targetRepo.path?.override) {
			...
			return targetRepo.path?.defaultName &&
				targetRepo.path.defaultName.length > 0
...
function createImagePath(
	...
	if (!sourceFrontmatter || !sourceFrontmatter.attachmentLinks) {
		if (settings.embed.useObsidianFolder) {
			if (settings.upload.behavior === FolderSettings.yaml) {
				return settings.upload.rootFolder.length > 0
					? normalizePath(`${settings.upload.rootFolder}/${filePath}`)
					: filePath;
			} else {
				//no root, but default folder name
				return settings.upload.defaultName.length > 0
					? normalizePath(`${settings.upload.defaultName}/${filePath}`)
					: filePath;
			}
		}
	...
	} else if (sourceFrontmatter?.attachmentLinks) {
