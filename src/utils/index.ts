...
export function monkeyPatchConsole(plugin: GithubPublisher) {
	...
	if (
		!stack?.includes('obsidian-mkdocs-publisher') ||
...
export async function createLink(
	...
	let filepath = getReceiptFolder(
		file,
		otherRepo,
		plugin,
		multiRepo.frontmatter
	);
...
async function publisherNotificationOneRepo(
	...
	if (file instanceof String) {
		successMsg = i18next.t('informations.successfulPublish', {
			nbNotes: noticeValue,
			repo,
		});
	} else {
		logs({ settings }, 'file published :', noticeValue);
		successMsg = i18next.t('informations.successPublishOneNote', {
			file: noticeValue,
			repo,
		});
	}
