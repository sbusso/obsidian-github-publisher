...
export class OverrideAttachmentsModal extends Modal {
	...
	onOpen(): void {
		...
		contentEl.createEl('h2', {
			text: i18next.t('settings.embed.overrides.modal.title'),
		});
		...
		ul.createEl('li', {
			text: i18next.t('settings.regexReplacing.modal.name'),
		});
		contentEl.createEl('h3', {
			text: i18next.t('settings.regexReplacing.modal.force'),
		});
		contentEl.createEl('p', {
			text: i18next.t('settings.embed.forcePush.info'),
		});
			...
			.addButton(button => {
				button.setButtonText(i18next.t('common.save')).onClick(() => {
					...
					this.allOverrides.forEach(override => {
						...
						const isForbiddenReplace = this.forbiddenValue(
							override.destination
						);
						canBeValidated.push(isForbiddenEntry.isForbidden);
						canBeValidated.push(isForbiddenReplace.isForbidden);
						if (
							isForbiddenEntry.isForbidden ||
							isForbiddenReplace.isForbidden
						) {
			...
			.addButton(button => {
				button.setButtonText(i18next.t('common.save')).onClick(() => {
					...
					this.allRegex.forEach(title => {
						...
						const isForbiddenEntry = this.forbiddenValue(
							title.regex,
							title.type
						);
...
export class ModalRegexOnContents extends Modal {
	...
	onOpen() {
		...
		for (const censorText of this.settings.conversion.censorText) {
			...
			const isFirstInList =
				this.settings.conversion.censorText.indexOf(censorText) === 0;
