...
export class TokenEditPath extends Modal {
	...
	onOpen() {
		...
		desc.createEl('br');
		desc.createEl('p', {
			text: i18next.t('settings.github.ghToken.button.variables'),
		});
		desc.createEl('ul', undefined, span => {
			span.createEl('li', undefined, li => {
				...
				li.createEl('span', undefined, e => {
					e.appendText(
						`${i18next.t('settings.github.ghToken.button.configDir')} (`
					);
			...
			span.createEl('li', undefined, li => {
				...
				li.createEl('span', undefined, e => {
					e.appendText(
						`${i18next.t('settings.github.ghToken.button.pluginID')} (`
					);
		...
		new Setting(contentEl).addButton(button => {
			button.setButtonText(i18next.t('common.save')).onClick(async () => {
				try {
					...
					input.controlEl.querySelector('input')!.style.border =
						'1px solid red';
