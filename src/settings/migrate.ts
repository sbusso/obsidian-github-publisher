...
async function migrateReplaceTitle(plugin: GithubPublisher) {
	...
	logs(
		...
		i18next.t('informations.migrating.fileReplace')
	);
...
async function migrateSubFolder(plugin: GithubPublisher) {
	...
	if (
		...
		logs(
			{ settings: plugin.settings },
			i18next.t('informations.migrating.subFolder')
		);
		//@ts-ignore
...
export async function migrateToken(
	...
	logs(
		{ settings: plugin.settings },
		`Moving the GitHub Token in the file : ${tokenPath} for ${repo ?? 'default'} repository`
	);
	...
	if (tokenPath.endsWith('.json')) {
		...
		if (!exists) {
			await plugin.app.vault.adapter.write(
				tokenPath,
				JSON.stringify(envToken, null, 2)
			);
			return;
		}
