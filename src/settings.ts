...
import {
	...
	ImportModal,
	loadAllPresets,
} from './settings/modals/import_export';
...
} from './settings/modals/regex_edition';
...
export class GithubPublisherSettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		if (
			this.settings.plugin.saveTabId !== undefined &&
			!this.settings.plugin.saveTabId
		) {
			//real false
			...
			.addButton(button => {
				button
					...
					.onClick(async () => {
						...
						const presetLists = await loadAllPresets(
							octokit.octokit,
							this.plugin
						);
	...
	renderUploadConfiguration() {
		...
		new Setting(this.settingsPage)
			...
			.addText(text => {
				text
					.setPlaceholder(
						i18next.t('settings.upload.defaultFolder.placeholder')
					)
		...
		const frontmatterKeySettings = new Setting(this.settingsPage)
			...
			.addText(text => {
				text
					.setPlaceholder(
						i18next.t('settings.upload.frontmatterKey.placeholder')
					)
	...
	renderTextConversion() {
			...
			.addButton(button => {
				button.setIcon('pencil').onClick(async () => {
					new ModalRegexOnContents(
						this.app,
						this.copy(this.settings),
						async result => {
							this.settings.conversion.censorText =
								result.conversion.censorText;
							await this.plugin.saveSettings();
						}
					).open();
				});
		...
		new Setting(this.settingsPage)
			...
			.addTextArea(text => {
				text
					.setPlaceholder(
						i18next.t('settings.conversion.tags.exclude.placeholder')
					)
	...
	async renderEmbedConfiguration() {
		...
		if (embedSettings.notes) {
			...
			if (embedSettings.convertEmbedToLinks === 'links') {
				...
				this.settingsPage.createEl('p', {
					text: i18next.t('settings.embed.bake.text'),
				});
	...
	renderPluginSettings() {
		...
		this.settingsPage.createEl('h3', {
			text: i18next.t('settings.plugin.head.share'),
		});
		new Setting(this.settingsPage)
		...
		this.settingsPage.createEl('h3', {
			text: i18next.t('settings.plugin.head.menu'),
		});
			...
			new Setting(this.settingsPage)
				...
				.addDropdown(dropdown => {
					dropdown
						...
						.setValue(
							pluginSettings.copyLink.transform.slugify as
								| 'disable'
								| 'strict'
								| 'lower'
						)
		...
		this.settingsPage.createEl('h3', {
			text: i18next.t('settings.plugin.head.other'),
		});
		...
		this.settingsPage.createEl('h4', {
			text: i18next.t('settings.plugin.head.log'),
		});
